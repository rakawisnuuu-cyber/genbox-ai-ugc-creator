...
export default function ApiKeySetupModal({ open, onClose }: Props) {
  ...
  const handleSaveAndFinish = async () => {
    const key = keys.kie_ai.key;
    if (!key.trim()) return;
    await saveKey("kie_ai", key);
    setStep("done");
  };
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-md bg-card border-border">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === "intro" && "Sebelum mulai generate, kamu perlu setup API key Kie AI."}
            {step === "kie" && "Satu key untuk semua fitur: gambar, video, dan prompt AI."}
            {step === "done" && "API key sudah tersimpan. Kamu siap mulai!"}
          </DialogDescription>
        </DialogHeader>
                ...
                <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/30 border border-border">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Satu key untuk generate gambar, video, dan prompt AI
                    </p>
                  </div>
                </div>
              </div>
          ...
          {step === "kie" && (
            <>
                ...
                <div className="bg-muted/30 border border-border rounded-lg p-3 space-y-2">
                  ...
                  <ol className="text-xs text-muted-foreground space-y-1 list-decimal list-inside">
                    <li>
                      ...
                      >
                        kie.ai <ExternalLink className="h-2.5 w-2.5" />
                      </a>
                    </li>
              ...
              <div className="flex gap-2">
                ...
                >
                  {savingProvider === "kie_ai" ? (
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
                  ) : (
                    <>
                      Simpan & Selesai <CheckCircle2 className="h-3.5 w-3.5" />
                    </>
                  )}
                </Button>
              </div>
          ...
          {step === "done" && (
            <>
              <div className="text-center py-4">
                ...
                <p className="text-sm text-foreground font-medium">API key berhasil disimpan!</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Kamu sudah bisa mulai generate gambar dan video UGC.
                </p>
              </div>
